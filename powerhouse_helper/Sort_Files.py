...
def sorting_function(path, path_root):
    for elem in os.listdir(path):
        ...
        if len(elem.split(".")) > 1:
            for extend, category in EXTENDS.items():
                if os.path.splitext(elem)[-1] in extend:
                    ...
                    new_path = path_root + category + normalize(elem)
            ...
            if os.path.splitext(elem)[-1] not in [extend for extends_tup in EXTENDS.keys() for extend in extends_tup]:
                current_file = path + "\\" + elem
                new_path = path_root + "\\other\\" + normalize(elem)
                counter["other"] += 1
                shutil.move(current_file, new_path)
        ...
        if (
            ...
            sorting_function(path + "\\" + elem, path_root)

    return "The folder has been sorted!"


def start_sorting():
    path = input("\033[1mEnter the path to the folder to sort: \033[0m")
    path_root = path
    try:
        create_folders(path)
        print(sorting_function(path, path_root))
