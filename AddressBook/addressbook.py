...
fh = logging.FileHandler("AddressBook\\address_saves\\address_book.log")
...
class AddressBook:
    ...
    def save(self):
        with open('AddressBook\\address_saves\\address_book_auto_save.bin', "wb") as fd:
    ...
    def load(self):
        try:
            with open('AddressBook\\address_saves\\address_book_auto_save.bin', "rb") as fd:
                if not os.stat('AddressBook\\address_saves\\address_book_auto_save.bin').st_size == 0:
                    self.data = pickle.load(fd)
                    logger.info(f"Address Book has been loaded")
                    return f"Address Book has been loaded from"
