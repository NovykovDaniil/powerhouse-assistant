...
class InputError:
    ...
    def __call__(self, contacts, *args):
        try:
            ...
            return 'Error! Incorrect argument! Try help!'
        except DateIsNotValid:
            return 'Error! Date is not valid! Format is 2000-01-01'
        except IndexError:
            return 'Error! Incorrect argument! Try help!'
...
def del_note(notebook, *args):
    id_note = int(args[0])
    yes_no = input(f'Are you sure you want to delete the note ID:{id_note}? (Yes/not) ')
    if yes_no == 'Yes':
...
def goodbye(notebook, *args):
    notebook.save()
    print('\033[034mYou have finished working with notebook.\033[0m')
...
def help_me(*args):
    return """\nCommand format:
    If you want to get help - help or ?;
    If you want to add note - add note <text>;
    If you want to change note - change note <id> <text>;
    If you want to delete note - delete note <id>;
    If you want to add/change date - add date <id> <date>;
    If you want to add tag - add tag <id> <tag>;
    ...
    If you want to show all notes sorted by tags - sort by tags;
    If you want to exit the program - good bye or close or exit or . """


COMMANDS = {help_me: ['?', 'help', 'h'], goodbye: ['buy', 'good bye', 'close', 'exit', '.'], add_note: ['add note ', '+', 'new '],
            add_date: ['add date '], show_all: ['show all'], show_archiv: ['show archived'],
            change_note: ['change note '], del_note: ['delete note ', 'remove note', 'del note'], find_note: ['find note ', 'find ', 'search '],
...
def start_prog():
    ...
    print('\033[034mWelcome to notebook helper!\033[0m')
    print(f"\033[033mTo get started try command or '?' for help \033[0m")
    while True:
        user_command = input(">>> ")
